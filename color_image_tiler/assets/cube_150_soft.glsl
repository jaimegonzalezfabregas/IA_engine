...
void main() {
    ...
    for(int i = 0; i < TILE_COUNT; ++i) {
        ...
        float base_x = i / TILE_COUNT_SQRT;
        float base_y = i % TILE_COUNT_SQRT;

        float grid_size = PARTICLE_FREEDOM / float(TILE_COUNT_SQRT);

        float x = point.x * grid_size + base_x / TILE_COUNT_SQRT;
        float y = point.y * grid_size + base_y / TILE_COUNT_SQRT;

        float x_d = x - gl_FragCoord.x / 640;
        float y_d = y - gl_FragCoord.y / 640;
